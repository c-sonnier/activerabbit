    ...
    def update_monitor_status(monitor, result)
      ...
      if previous_status != "pending" && previous_status != new_status
        if new_status == "down" && previous_status != "down"
          Uptime::AlertJob.perform_async(monitor.id, "down", { consecutive_failures: monitor.consecutive_failures })
        elsif new_status == "up" && previous_status == "down"
          Uptime::AlertJob.perform_async(monitor.id, "up", { previous_status: previous_status })
        end
      end
    end
